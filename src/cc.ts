...
export function activate(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    commands.registerCommand('smartsemicolon.toggleAutoLineChange', toggleAutoLineChange)
  )
}
  ...
  editor
    .edit((editBuilder) => {
      for (let selection of editor.selections) {
        ...
        let position = parser
          ? parser.getSemicolonPosition(line, selection.active)
...
function updateConfiguration() {
  ...
  autoLineChange = config.get('autoLineChange', true)
  acceptSuggestions = config.get('acceptSuggestions', true)
  showInStatusBar = config.get('showInStatusBar', true)
  deleteEmptyLine = config.get('deleteEmptyLine', true)
...
const clangFamilyParser = new LanguageParser(
  '//',
  ['{', '}'],
  ['for'],
